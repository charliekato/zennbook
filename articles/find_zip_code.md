...
エクセルがなくてもプログラムは動くと思います。
# データベース作成
## 郵便番号csvのダウンロード
データベースはエクセルにしました。アクセスにするほどの事ではないのでそうしました。エクセルなら大抵の方はお持ちでしょうから。プログラム本体はエクセルのファイルは必要ですが、エクセルそのものはいらないと思います。データベースを作るのも面倒もしくは、エクセルそのものをお持ちでない方はgithubにいれてあるのでそこからダウンロードしてお使いください。
...
エクセルにはもう一枚シートを作り市へとの名前をprefにします。このシートには県名と県番号の対応表を作ります。これも「県番号」でググったら出てきますのでそれを使って下の図のように作ります。見出し行は県名を pref 県番号を　prefNo　としておいてください。
![](/images/findZipCode/prefNo.jpg =360x)

# プログラム本体
データベースができたのでプログラムです。プログラムはvisual studioで作りましたが、visual studioからプログラムのリリースをどうやるのかわからいので(すいません。)できたソースコードを一つのファイルにまとめました。githubからダウンロードできます。ここにもコードを載せます。プログラムの説明は割愛させていただきます。
```c#
...
```
プログラムはcscでコンパイルできます。exeファイルもgithubからだアンロードできます。
## 設定ファイル
findZipCode.ini という名前の設定ファイルが必要です。データベースとして使うエクセルファイルの場所をフルパスで指定する必要があります。以下がその例です。
```
#####
# Excel FIle をfull pathで指定してください。
