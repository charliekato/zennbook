...
私のSurfaceでは今一つこのnumlockキーがうまく動作しません。他のマシンでどうなのかまだ調査していませんが、今のところ原因が突き止められていません。しかたがなく[numlocklock](https://www.inasoft.org/numlklk/)というアプリを使っています。
# 外観
...
```
キーコードをホストに送るには、次のようにKeyboard.send()という関数を使います。引数はキーコードです。キーコードはusb_hidのライブラリーに定義されています。例えば数字の 1 のキーコードはKeycode.ONE なので、それをホストに送るには次のようにします。
```py
kbd = Keyboard(usb_hid.devices)
kbd.send(Keycode.ONE)
```
実に簡単ですよね。
表1をもう一度見てください。一番上のkeyNoというのはボタンスイッチを押したときのラズパイピコの入出力信号からプログラムの内部でどのキーが押されたかを示す値で、rowValueとcolumnからpythonで計算します。それについては後ほどせ説明します。このkeyNoを添え字(index)にしたアレイkeycodeを次のように定義します。これが表1の一番下のKeycodeに対応します。
...
# ホストに送信 (keyin()関数を作る)
キーを取り込んだらPCに信号を送る必要があります。これは usb_hidというライブラリーを使います。それではmainです。mainはいたってかんだんです。次のようになります。
